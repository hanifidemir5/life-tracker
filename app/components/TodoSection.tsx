...
const CountdownTimer = () => {
  ...
  useEffect(() => {
    const updateTimer = () => {
      const now = new Date();
      let target = new Date(now.getFullYear(), 8, 6); // month is 0-indexed, 8 = September
      if (now > target) {
  ...
  return (
    <div className="bg-gradient-to-r from-rose-500 to-pink-500 rounded-2xl shadow-md p-4 text-white flex items-center justify-between border border-rose-400">
      <div className="flex items-center gap-2">
        <div className="p-2 bg-white/20 rounded-full">
          <CalendarClock className="w-5 h-5 text-white" />
        </div>
        <span className="font-bold text-sm tracking-wide">
          Lisans KPSS'ye Kalan Süre
        </span>
      </div>
      <span className="font-black text-lg tracking-tight bg-white/20 px-3 py-1 rounded-xl">
        {timeLeft}
      </span>
    </div>
  );
...
export default function CalendarSidebar({
  ...
  const { data: dueTodos = [], isLoading } = useTodosDueOn(
    userId,
    selectedDate,
  );
  ...
  const isFutureOrToday = (d: number) => {
  ...
  return (
    <aside
      className={`w-full shrink-0 lg:w-80 xl:w-[340px] self-start sticky top-24 ${className}`}
    >
        ...
        <div className="bg-white rounded-4xl shadow-sm p-5 border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between mb-4 px-2">
            <h3 className="font-extrabold text-gray-800 text-lg tracking-tight capitalize">
              {viewDate.toLocaleDateString(t("locale") || "en-US", {
                month: "long",
              })}{" "}
              {year}
            </h3>
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-full transition-colors ml-1"
              >
                {isCalendarOpen ? (
          ...
          <div
            className={`transition-all duration-300 overflow-hidden ${isCalendarOpen ? "max-h-80 opacity-100" : "max-h-0 opacity-0"}`}
          >
            <div className="grid grid-cols-7 mb-2">
              {(t("weekdays") || "Su,Mo,Tu,We,Th,Fr,Sa").split(",").map((d) => (
                <div
                  key={d}
                  className="text-center text-gray-400 text-xs font-bold py-1 tracking-wider uppercase"
                >
                  {d}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7 gap-y-2 gap-x-1">
              {calendarCells.map((day, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center justify-center relative h-10"
                >
                  {day ? (
                    <>
                      <button
                      ...
                      <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-0.5">
                        {(() => {
                          ...
                          const hasO = dots?.some(
                            (t) =>
                              t.period === "one-time" && isTodoDueOn(t, iso),
                          );
                          const hasD = dots?.some(
                            (t) => t.period === "daily" && isTodoDueOn(t, iso),
                          );
                          const hasM = dots?.some(
                            (t) =>
                              t.period === "monthly" && isTodoDueOn(t, iso),
                          );
                          const hasY = dots?.some(
                            (t) => t.period === "yearly" && isTodoDueOn(t, iso),
                          );
                          return (
                            <>
                              {hasO && (
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between px-2">
            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">
              {t("upcomingDue") || "Upcoming Due"}
            </h4>
            <span className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2.5 py-1 rounded-full">
              {myDueTodos.length + partnerDueTodos.length}{" "}
              {t("tasks") || "Tasks"}
            </span>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 text-center">
                <p className="text-sm text-gray-400 font-medium">
                  {t("nothingDueOnDay") || "Nothing due on this day"}
                </p>
              </div>
            ) : (
              <>
                {myDueTodos.map((todo) => (
                  <div
                    key={todo.id}
                    className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4 flex items-center gap-3 relative overflow-hidden group"
                  >
                    {/* Color accent left line */}
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-1 ${todo.period === "one-time" ? "bg-gray-400" : todo.period === "daily" ? "bg-rose-400" : todo.period === "monthly" ? "bg-blue-400" : "bg-amber-400"}`}
                    />

                    <div className="flex-1 min-w-0 pl-2">
                      <h5
                        className={`text-base font-bold truncate leading-tight mb-1 flex items-center gap-1 ${todo.done ? "text-gray-800" : "text-gray-400"}`}
                      >
                        {todo.text}
                        <span
                          className={`text-[10px] ml-1 px-1.5 py-0.5 rounded-full ${todo.done ? "bg-gray-100 text-gray-400" : "bg-rose-50 text-rose-400"}`}
                        >
                          {profiles[todo.user_id] || t("me") || "Me"}
                        </span>
                      </h5>
                      <p className="text-xs text-gray-400 font-medium">
                        {todo.period === "daily"
                          ? t("dueTomorrow") || "Due Tomorrow"
                          : `${t("due") || "Due"} ${selectedDate.toLocaleDateString(t("locale") || "en-GB", { weekday: "long" })}`}
                      </p>
                    </div>
                    <button
                      onClick={() =>
                      ...
                      className="text-rose-400 hover:text-rose-500 hover:bg-rose-50 p-2 rounded-xl transition-colors shrink-0"
                    >
                      {todo.done ? (
                        <CheckCircle2 className="w-6 h-6 fill-rose-100" />
                      ) : (
                        <CalendarIcon className="w-5 h-5 text-gray-300 group-hover:text-rose-400" />
                      )}
                    </button>
                    <button
                      onClick={() =>
                      ...
                      className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-rose-500 p-1"
                    >
                ...
                {partnerDueTodos.map((todo) => (
                  <div
                    key={todo.id}
                    className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4 flex items-center gap-3 relative overflow-hidden opacity-70"
                  >
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-1 ${todo.period === "one-time" ? "bg-gray-400" : todo.period === "daily" ? "bg-rose-400" : todo.period === "monthly" ? "bg-blue-400" : "bg-amber-400"}`}
                    />
                    <div className="flex-1 min-w-0 pl-2">
                      <h5 className="text-base font-bold truncate leading-tight mb-1 flex items-center gap-1 text-gray-600">
                        {todo.text}
                        <span className="text-[10px] text-purple-400 ml-1 bg-purple-50 px-1.5 py-0.5 rounded-full">
                          {profiles[todo.user_id] ||
                            t("partnerTodos") ||
                            "Partner"}
                        </span>
                      </h5>
          ...
          <div className="bg-white rounded-4xl shadow-sm p-4 border border-gray-100 flex flex-col gap-4">
            ...
            {canAdd ? (
              ...
              <div className="flex items-center gap-2 px-3 py-6 justify-center">
                ...
                <p className="text-sm text-gray-400 font-medium">
                  {t("pastDayLocked") || "Cannot add goals to past days"}
                </p>
              </div>
            )}
          </div>
...
function DueRow({
  ...
  return (
    <li className="flex items-center gap-2 group rounded-lg px-1 py-0.5 hover:bg-gray-50 transition-colors">
      ...
      <span
        className={`shrink-0 text-[9px] font-bold uppercase tracking-wider border rounded px-1 py-0.5 ${periodBadge[todo.period]}`}
      >
        {todo.period.charAt(0).toUpperCase()}
      </span>
      <span
        className={`flex-1 text-sm leading-tight ${todo.done ? (isOwn ? "text-gray-600" : "text-purple-400 italic") : "text-gray-300"}`}
      >
        {todo.text}
      </span>
      {isOwn && (
        <button
          onClick={onDelete}
          className="opacity-0 group-hover:opacity-100 text-gray-200 hover:text-rose-400 transition-all"
        >
