...
import { useSearchItems, useItems, useInvalidateItems, useAllItems, Item, ITEMS_PER_PAGE } from "@/app/hooks/useItems";
...
export default function SearchPage() {
  ...
  // React Query hooks
  const { data: paginatedData, isLoading: itemsLoading } = useSearchItems(query, currentCategoryKey, currentPage);
  const { data: categoryData, isLoading: categoryLoading } = useCategoryByKey(currentCategoryKey !== "all" ? currentCategoryKey : "");
  ...
  const headerTitle = query ? `${t('search')} : ${query}` : t('search');
    ...
    <main className="min-h-screen p-8 pb-32">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-start lg:justify-between">
        ...
        <div className="grid grid-cols-1 gap-4 z-0">
          {/* SEARCH PAGE NO ADD BUTTON HERE */}
